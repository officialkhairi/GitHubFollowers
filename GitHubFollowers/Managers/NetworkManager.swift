...
class NetworkManager {
    ...
    func getFollwers(for username: String, page: Int, completed: @escaping ([Follower]?, String?) -> Void) {
        let endpint = baseURL + "\(username)/followers?per_page=100&page=\(page)"
        
        guard let url = URL(string: endpint) else {
        ...
        let task = URLSession.shared.dataTask(with: url) { data, response, error in
            ...
            if let _ = error {
                completed(nil, "Unable to complete your request. Please check your internet connnection.")
            }
